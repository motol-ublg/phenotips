...
import org.phenotips.studies.family.migrations.PhenotipsFamilyMigrations;
...
public class R71492PhenoTips2155DataMigration extends AbstractHibernateDataMigration
{
    ...
    {
        return "Migrate old family studies data.";
    }
    ...
    private class MigratePedigreeCallback implements HibernateCallback<Object>
    {
            ...
            Map<String, XWikiDocument> relativesDocList) throws QueryException, XWikiException, Exception
        {
            ...
            JSONObject pedigreeData = null;
            JSONObject pedigreeImage = null;
            if (pedigreeData.isNullObject() || pedigreeData.isEmpty()) {
        ...
        {
            List<String> membersRefsList = Arrays.asList(patientXDocument.getDocumentReference().getName());
            ...
            throws XWikiException
        {
            Query rq = session.createQuery("select distinct o.name from BaseObject o,"
                + " StringProperty eid where o.className = '"
                + migrator.serializer.serialize(Patient.CLASS_REFERENCE)
                + "' and eid.id.name = 'external_id' "
                + "and o.name <> 'PhenoTips.PatientTemplate' and eid.id.id = "
                + relativeDoc);
